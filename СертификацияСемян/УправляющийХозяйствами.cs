...
public class УправляющийХозяйствами
{
    ...
    public List<ПроизводительСемян> ПолучитьХозяйстваПользователя(string идПользователя)
    {
        return контекст.ПроизводителиСемян
            .AsNoTracking()
            .Where(_ => _.ИдВладельца == идПользователя && _.ДатаУдаления == null).ToList();
    }

    public List<УчастокПоля> ПолучитьУчасткиПоля(int идПоля)
    {
        return контекст.УчасткиПолей
            .AsNoTracking()
            .Where(_ => _.ИдПоля == идПоля && _.ДатаУдаления == null).ToList();
    }

    public List<Поле> ПолучитьПоляХозяйства(int идХозяйства)
    {
        return контекст.Поля
            .AsNoTracking()
            .Where(_ => _.ИдХозяйства == идХозяйства && _.ДатаУдаления == null).ToList();
    }
    ...
    public ПроизводительСемян? ПолучитьХозяйство(int идХозяйства)
    {
        return контекст.ПроизводителиСемян
            .AsNoTracking()
            .FirstOrDefault(_ => _.Ид == идХозяйства && _.ДатаУдаления == null);
    }
    ...
    public Поле ПолучитьПоле(int идПоля)
    {
        return контекст.Поля
            .AsNoTracking()
            .Where(_ => _.Ид == идПоля).First();
    }

    public УчастокПоля? ПолучитьУчасток(int идУчастка)
    {
        return контекст.УчасткиПолей
            .AsNoTracking()
            .FirstOrDefault(_ => _.Ид == идУчастка && _.ДатаУдаления == null);
    }

    public void ДобавитьУчасток(int идПоля, string название, string координаты)
    {
        контекст.УчасткиПолей.Add(new УчастокПоля()
        {
            ИдПоля = идПоля,
            Название = название,
            КоординатыУчастка = координаты,
            Активно = true,
        });
        контекст.SaveChanges();
    }

    public void ИзменитьУчасток(int идУчастка, string название, string координаты)
    {
        var поле = контекст.УчасткиПолей.FirstOrDefault(_ => _.Ид == идУчастка && _.ДатаУдаления == null);
        if (поле is null)
        {
            return;
        }
        ...
        поле.Название = название;
        поле.КоординатыУчастка = координаты;
        контекст.SaveChanges();
    }

    public void УдалитьУчасток(int идУчастка)
    {
        контекст.УчасткиПолей
            .Where(_ => _.Ид == идУчастка)
            .ExecuteUpdate(_ => _.SetProperty(_ => _.ДатаУдаления, DateTime.UtcNow));
    }
}
